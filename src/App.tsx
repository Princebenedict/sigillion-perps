...
const SLogo = ({ size = 14 }: { size?: number }) => (
  <span className="s-logo" style={{ width: size, height: size, fontSize: size * 0.6 }}>S</span>
);
...
export default function App() {
  ...
  const drawCandlestickChart = useCallback((container: HTMLDivElement) => {
    ...
    candles.forEach((c, i) => {
      ...
      const bodyTop = toY(Math.max(c.o, c.c)); const bodyH = Math.max(1, Math.abs(toY(c.o) - toY(c.c)));
      ctx.fillStyle = col; ctx.globalAlpha = isUp ? 0.85 : 0.9; ctx.fillRect(x, bodyTop, barW, bodyH); ctx.globalAlpha = 1;
      ctx.strokeStyle = col; ctx.lineWidth = 0.5; ctx.strokeRect(x, bodyTop, barW, bodyH);
    });
    const handleMM = (e: MouseEvent) => {
      const rect = canvas.getBoundingClientRect();
      const mx = e.clientX - rect.left; const my = e.clientY - rect.top;
      const img = ctx.getImageData(0, 0, canvas.width, canvas.height); ctx.putImageData(img, 0, 0);
      ctx.strokeStyle = 'rgba(0,245,196,0.25)'; ctx.lineWidth = 1; ctx.setLineDash([4,4]);
      ...
      if (label) { label.style.display = 'block'; label.style.top = my + 'px'; label.textContent = fmt(Math.max(0, hoverPrice)); }
    };
    const handleML = () => { if (crosshairRef.current) crosshairRef.current.style.display = 'none'; };
    canvas.addEventListener('mousemove', handleMM);
    canvas.addEventListener('mouseleave', handleML);
    (canvas as any)._cleanup = () => { canvas.removeEventListener('mousemove', handleMM); canvas.removeEventListener('mouseleave', handleML); };
  ...
  const buildLineChart = useCallback(() => {
    ...
    grad.addColorStop(0, 'rgba(0,245,196,0.20)'); grad.addColorStop(1, 'rgba(0,245,196,0)');
    const ChartCtor = (window as any).Chart; if (!ChartCtor) return;
    ...
    canvas.addEventListener('mousemove', (e) => {
      const label = crosshairRef.current; if (!label || !chartInstance.current) return;
      const rect = canvas.getBoundingClientRect(); const y = e.clientY - rect.top;
      const yScale = chartInstance.current.scales?.y; if (!yScale) return;
      label.style.display = 'block'; label.style.top = y + 'px'; label.textContent = fmt(yScale.getValueForPixel(y));
    });
    canvas.addEventListener('mouseleave', () => { if (crosshairRef.current) crosshairRef.current.style.display = 'none'; });
  }, [market, timeframe, tick.basePrice]); // eslint-disable-line
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      setActiveTraders(prev => { const n = Math.max(8000, prev + Math.floor(Math.random() * 480 - 240)); setTradersFlash(n > prev ? 'up' : 'dn'); setTimeout(() => setTradersFlash(''), 1200); return n; });
      setTxPerMin(prev => { const n = Math.max(1000, prev + Math.floor(Math.random() * 1200 - 600)); setTxnsFlash(n > prev ? 'up' : 'dn'); setTimeout(() => setTxnsFlash(''), 1200); return n; });
    }, 9000);
    return () => clearInterval(iv);
  }, []);
  ...
  const toggleChartExpand = () => {
    ...
    setTimeout(() => { if (chartInstance.current) chartInstance.current.resize(); else buildChart(); }, 50);
  };
  ...
  async function handleOpenPosition() {
    ...
    if (balance !== null && sizeUsd / tick.price > balance) { setTxLog(`Insufficient balance. Available: ${balance.toFixed(4)} SOL`); return; }
    setLoading(true);
  ...
  function handleRevealPosition(id: number) {
    revealPosition(id, () => setPositions(prev => prev.map(p => p.id === id ? { ...p, encrypted:false } : p)));
  }
  ...
  function handleAddToPosition(id: number) {
    ...
    const add = parseFloat(raw); if (isNaN(add) || add <= 0) return;
          ...
          return (
            <div key={p.id} className="pos-card pos-card-revealed">
              ...
              <div className="pc-actions">
                ...
                <button className="pos-btn hide-b"  onClick={() => handleHidePosition(p.id)} title="Hide position"><IconEyeOff /></button>
          ...
          <table className="pos-table">
            <thead>
              <tr>
                ...
                <th>Entry</th><th>Exit</th><th>PNL ($)</th><th>PNL (%)</th><th>Time</th><th></th>
              </tr>
            </thead>
            <tbody>
              {history.map(h => {
                ...
                <div className="hmc-top">
                  <div className="hmc-left">
                    ...
                    <div className="hmc-badges">
                      ...
                      <span className={`hmc-status ${isUp ? 'hmc-profit' : 'hmc-loss'}`}>
                        {h.status === 'LIQUIDATED' ? 'LIQ' : isUp ? 'PROFIT' : 'LOSS'}
                      </span>
                    </div>
                  </div>
      ...
      <div className="op-body">
        ...
        <div className="field">
          ...
          <div className="lev-row">
            {[2, 5, 10, 20, 50].map(l => (
              <button key={l} className={`lev-btn ${leverage === l ? 'lev-on' : ''}`} onClick={() => setLeverage(l)}>{l}x</button>
            ))}
          </div>
        </div>
            ...
            {positions.map(p => {
              ...
              return (
                <div key={p.id} className="dt-row" style={COL_POS}>
                  ...
                  <span className={`pnl-cell ${up ? 'up' : 'dn'}`}>{p.encrypted ? '— encrypted' : `${up ? '+' : ''}$${Math.abs(p.pnl).toFixed(2)}`}</span>
            ...
            <section className={`chart-area ${isChartExpanded ? 'chart-area-expanded' : ''}`}>
              <div className="chart-header-hud">
                ...
                <div className="hud-timeframes">
                  {TIMEFRAMES.map(tf => (
                    <button key={tf} className={`hud-tf-btn ${timeframe === tf ? 'active' : ''}`} onClick={() => handleTFChange(tf)}>{tf}</button>
                  ))}
                  <div className="chart-type-divider"/>
                  <button className={`hud-tf-btn chart-type-btn ${chartType === 'candlestick' ? 'active' : ''}`} onClick={() => setChartType('candlestick')} title="Candlestick">CNDL</button>
                  <button className={`hud-tf-btn chart-type-btn ${chartType === 'line' ? 'active' : ''}`} onClick={() => setChartType('line')} title="Line">LINE</button>
                </div>
                <div className="hud-actions">
                  <button className="fs-btn desktop-only" onClick={toggleFullScreen} title="Fullscreen">[ ]</button>
                  <button className="fs-btn mobile-only" onClick={toggleChartExpand} title={isChartExpanded ? 'Collapse' : 'Expand'}>
                    {isChartExpanded ? 'COLL' : 'EXP'}
                  </button>
